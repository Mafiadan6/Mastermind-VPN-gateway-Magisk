...
# === UNIVERSAL ICMP EXEMPTION: ALLOW ALL PRIVATE IPs ===
PREROUTING -t nat -s 192.168.0.0/16 -p icmp -j ACCEPT
PREROUTING -t nat -s 10.0.0.0/8 -p icmp -j ACCEPT
PREROUTING -t nat -s 172.16.0.0/12 -p icmp -j ACCEPT

# === UNIVERSAL FALLBACK FOR PRIVATE NETWORKS ===
...
function setup() {
    ...
    # ===================================================================
    # WORKING BASE + STREAMING OPTIMIZATION
    # ===================================================================
    # 1. MTU that was working
    ip link set dev $tun_name mtu 1400 2>/dev/null
    echo "MTU: Set $tun_name to 1400"
    
    # 2. USB interface MTU
    ip link set dev usb0 mtu 1500 2>/dev/null
    
    # 3. SINGLE STREAMING FIX: TCP buffers for video
    echo '8192 87380 4194304' > /proc/sys/net/ipv4/tcp_rmem
    echo '8192 87380 4194304' > /proc/sys/net/ipv4/tcp_wmem
    echo "Applied streaming-optimized TCP buffers"
    # ===================================================================
